...
from experiments.seq2seq_ablation.model_c_tfdecay import build_model, compute_teacher_forcing_ratio


def parse_args():
    parser = argparse.ArgumentParser(description="Ablation C: teacher forcing decay.")
    add_base_train_args(
        parser=parser,
        default_model_path="./models/ablation_c_tfdecay",
        exp_name="ablation_c_tfdecay",
    )
    parser.add_argument("--tf_start", type=float, default=0.7)
    parser.add_argument("--tf_end", type=float, default=0.1)
    parser.add_argument("--tf_decay_epochs", type=int, default=10)
...
def main():
    ...
    logger.info("TF schedule: start=%.3f end=%.3f decay_epochs=%d", args.tf_start, args.tf_end, args.tf_decay_epochs)
    ...
    for epoch in range(args.epochs):
        ...
        logger.info("Epoch %d teacher_forcing_ratio=%.4f", epoch + 1, tf_ratio)
