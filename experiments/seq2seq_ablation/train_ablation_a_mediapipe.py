...
from experiments.seq2seq_ablation.model_a_mediapipe import build_model


def parse_args():
    parser = argparse.ArgumentParser(description="Ablation A: MediaPipe offline feature fusion.")
    add_base_train_args(
        parser=parser,
        default_model_path="./models/ablation_a_mediapipe",
        exp_name="ablation_a_mediapipe",
    )
    parser.add_argument("--pose_feature_root", type=str, default="./features/mediapipe")
    parser.add_argument("--pose_dim", type=int, default=256)
    parser.add_argument("--pose_fuse_weight", type=float, default=0.2)
...
def main():
    ...
    logger.info("Pose root: %s | pose_dim=%d | fuse_weight=%.3f", args.pose_feature_root, args.pose_dim, args.pose_fuse_weight)
    logger.info("Args: %s", vars(args))

    train_set, val_set, test_set, train_loader, val_loader, test_loader = build_dataloaders(
        args,
        use_pose=True,
        pose_root=args.pose_feature_root,
    ...
    logger.info("Missing pose feature fallback is enabled (zero vector).")
    ...
    model = build_model(
        ...
        device=device,
        pose_dim=args.pose_dim,
        pose_fuse_weight=args.pose_fuse_weight,
    )
