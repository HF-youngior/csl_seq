...
"""
Enhanced Sequence to Sequence Model with Attention
...
class Encoder(nn.Module):
    ...
    增强版编码器：ResNet + LSTM
    """
    def __init__(self, lstm_hidden_size=512, arch="resnet18", dropout=0.3):
...
class Decoder(nn.Module):
    ...
    增强版解码器：LSTM + 注意力机制
    """
    def __init__(self, output_dim, emb_dim, enc_hid_dim, dec_hid_dim, dropout=0.3):
        ...
        # LSTM层（增加dropout）
        self.rnn = nn.LSTM(emb_dim + enc_hid_dim, dec_hid_dim, dropout=dropout)
...
if __name__ == '__main__':
    # test encoder
    encoder = Encoder(lstm_hidden_size=512, dropout=0.3)
    ...
    # test decoder
    decoder = Decoder(output_dim=500, emb_dim=256, enc_hid_dim=512, dec_hid_dim=512, dropout=0.3)
    ...
    seq2seq = Seq2Seq_v2(encoder=encoder, decoder=decoder, device=device)
    imgs = torch.randn(16, 3, 8, 128, 128)
