...
from Seq2Seq_2plus1D import Seq2Seq_2plus1D, Encoder, Decoder
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='连续手语识别 - (2+1)D ResNet + LSTM模型')
    ...
    parser.add_argument('--model_path', type=str, default='./models/continuous_2plus1d', help='模型保存路径')
    ...
    parser.add_argument('--sample_duration', type=int, default=48, help='采样持续时间')
    parser.add_argument('--enc_hid_dim', type=int, default=512, help='编码器隐藏维度')
    parser.add_argument('--dec_hid_dim', type=int, default=512, help='解码器隐藏维度')
    ...
    parser.add_argument('--arch', type=str, default='resnet18', choices=['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152'], help='ResNet架构')
    ...
    # 初始化TensorBoard
    writer = SummaryWriter(log_dir=os.path.join('./runs', f'continuous_slr_2plus1d_{datetime.now().strftime("%Y%m%d_%H%M%S")}'))
    ...
    model = Seq2Seq_2plus1D(encoder=encoder, decoder=decoder, device=device).to(device)
    ...
    if args.mode == 'train':
        print('开始训练连续手语识别模型...')
        print(f'使用{args.arch.upper()}作为特征提取器')
