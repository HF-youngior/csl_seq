...
def _normalize_gt_tokens(tokens, token_unit, source_path):
    if token_unit == "char":
        return _tokenize_as_char(tokens)
    if token_unit == "word":
        return tokens
    # auto mode: fallback to char-level for CSL100 sentence-level labels
    if len(tokens) == 1 and _contains_cjk(tokens[0]) and "csl100" in source_path.lower():
        return _tokenize_as_char(tokens)
    return tokens


def load_groundtruth(fpath, token_unit="auto"):
    file_info = open(fpath, 'r', encoding='utf-8').readlines()
    gt_dict = dict()
    for line in file_info:
        info = line[:-1].split(" ")[5:]
        info = [*filter(lambda x: len(x), info)]
        info = _normalize_gt_tokens(info, token_unit, fpath)
...
def wer_calculation(gt_path, primary_pred, auxiliary_pred=None, token_unit="auto"):
    gt = load_groundtruth(gt_path, token_unit=token_unit)
