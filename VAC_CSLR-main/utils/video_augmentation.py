...
class RandomRotation(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            if scipy_misc is not None:
                rotated = [scipy_misc.imrotate(img, angle) for img in clip]
            else:
...
class RandomResize(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            if scipy_misc is not None:
                return [scipy_misc.imresize(img, size=(new_h, new_w), interp=self.interpolation) for img in clip]
            return [np.array(PIL.Image.fromarray(img).resize((new_w, new_h))) for img in clip]
        elif isinstance(clip[0], PIL.Image.Image):
