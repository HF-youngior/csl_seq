...
from models.Seq2Seq_v2 import Encoder, Decoder, Seq2Seq_v2
...
def main():
    parser = argparse.ArgumentParser(description='连续手语识别训练和测试 - 增强版')
    ...
    parser.add_argument('--corpus_path', type=str, default='corpus.txt',
                       help='语料库文件路径')
    parser.add_argument('--model_path', type=str, default='./models/continuous_v2',
                       help='模型保存路径')
    ...
    # 初始化TensorBoard
    writer = SummaryWriter(log_dir=os.path.join('./runs', f'continuous_slr_v2_{datetime.now().strftime("%Y%m%d_%H%M%S")}'))
    ...
    # 初始化模型
    encoder = Encoder(lstm_hidden_size=args.enc_hid_dim, arch=args.arch, dropout=args.dropout)
    ...
    model = Seq2Seq_v2(encoder=encoder, decoder=decoder, device=device).to(device)
    ...
    class LabelSmoothingLoss(nn.Module):
        ...
        def forward(self, pred, target):
            ...
            with torch.no_grad():
                ...
                smooth_target.scatter_(1, target, 1.0 - self.smoothing)
            
            # 计算损失
            loss = -torch.sum(smooth_target * log_preds, dim=1)
            
            # 忽略填充标记
            mask = (target.squeeze(1) != 0)
    ...
    if args.mode == 'train':
        ...
        print('开始训练连续手语识别模型...')
        print('='*50)
        print(f'使用带注意力机制的Seq2Seq_v2模型')
        print(f'数据增强: 随机水平翻转、颜色抖动、随机旋转')
        print(f'标签平滑: {args.label_smoothing}')
        print(f'学习率调度: 余弦退火')
        print('='*50 + '\n')
